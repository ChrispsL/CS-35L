...
static double camera_fov, pixel_dx, pixel_dy, subsample_dx, subsample_dy;
...
static float scaled_color[width][height][3];

void* parallel(void* arg)
{
    int nthThread = *(int*) arg;
    /* for every pixel */
    for( int px=nthThread; px<width; px+=nthreads )
    {
...
/*
	...
	for(int i = 0; argv[1][i] != 0; i++)
    	{
		fprintf(stdout, argv[1][i]);
		if('9' - argv[1][i] > 9)	// if not a number
		{
		fprintf(stderr, "Error in argument input. Only use integers\n");
		exit(1);
		}
    	}	
    }
*/
    ...
    int* numThread = (int*) malloc(sizeof(int) * nthreads);
    if(threads == 0 || numThread == 0)
    {
    ...
    {
	numThread[i] = i;
	if(pthread_create(&threads[i], NULL, parallel, &numThread[i]))
	{
